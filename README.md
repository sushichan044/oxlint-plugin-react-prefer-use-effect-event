...
An [Oxlint](https://oxc.rs/docs/guide/usage/linter.html) / [ESLint](https://eslint.org/) plugin that suggests wrapping calls to event handlers of your choice inside React effect hooks with [`useEffectEvent`](https://react.dev/reference/react/experimental_useEffectEvent).

## What it does

Given a `targets` config that marks `notify` from `some-notification-library` as a handler, the plugin flags calls to `notify` inside React effect hooks and suggests wrapping them with `useEffectEvent`.
...
| Name                                                             | Description                                                                                                                      | 🔧  |
| :--------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------- | :-- |
| [prefer-use-effect-event](docs/rules/prefer-use-effect-event.md) | Wrap event handlers passed into React effect hooks with `useEffectEvent` to avoid stale closures and unnecessary effect re-runs. | 🔧  |
