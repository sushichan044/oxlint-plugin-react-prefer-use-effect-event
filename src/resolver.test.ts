...
describe("findOxlintConfigDir", () => {
  it("returns the directory of the nearest .oxlintrc.json above the file", async () => {
    ...
    expect(r.findOxlintConfigDir(fx.getPath("packages/foo/src/index.ts"))).toBe(fx.path);
  });
  ...
  it("prefers the closer config when nested configs exist", async () => {
    ...
    expect(r.findOxlintConfigDir(fx.getPath("packages/foo/src/index.ts"))).toBe(
  ...
  it("recognises oxlint.config.* alongside .oxlintrc.json", async () => {
    ...
    expect(r.findOxlintConfigDir(fx.getPath("src/index.ts"))).toBe(fx.path);
  });
});
...
describe("resolveImportSource", () => {
  it("resolves a TS path alias through tsconfig paths", async () => {
    ...
    expect(r.resolveImportSource(fx.getPath("src/Component.tsx"), "@/hooks/useToast")).toBe(
  ...
  it("resolves a relative import to the same absolute path as the alias", async () => {
    ...
    expect(r.resolveImportSource(file, "@/hooks/useToast")).toBe(target);
    expect(r.resolveImportSource(file, "../hooks/useToast")).toBe(target);
  });
  ...
  it("returns null when the import cannot be resolved", async () => {
    ...
    expect(r.resolveImportSource(fx.getPath("src/Component.tsx"), "@/missing/path")).toBeNull();
  });
});
...
describe("findTsconfig", () => {
  it("walks upward to the nearest tsconfig.json", async () => {
    ...
    expect(r.findTsconfig(fx.getPath("packages/foo/src/index.ts"))).toBe(
      fx.getPath("tsconfig.json"),
    );
  });
  ...
  it("returns null when no tsconfig is reachable", async () => {
    ...
    expect(r.findTsconfig(fx.getPath("src/index.ts"))).toBeNull();
  });
});
